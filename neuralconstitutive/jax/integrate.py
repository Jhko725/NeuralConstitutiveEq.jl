...
def integrate_to(x_upper: float, x: Array, y: Array) -> float:
    return jax.lax.cond(x_upper > x[0], _integrate_to, lambda *_: 0.0, x_upper, x, y)


def _integrate_to(x_upper: float, x: Array, y: Array) -> float:
...
def integrate_from(x_lower: float, x: Array, y: Array) -> float:
    return jax.lax.cond(x_lower < x[-1], _integrate_from, lambda *_: 0.0, x_lower, x, y)


def _integrate_from(x_lower: float, x: Array, y: Array) -> float:
