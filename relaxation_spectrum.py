...
def DLNM_Integrand(t_, t, phi, velocity, indentation, tip):
    a = tip.alpha
    b = tip.beta
    return a * b * phi(t - t_) * velocity(t_) * indentation(t_) ** (b - 1)
...
def Quadrature(t1, t_, t_max_, phi_, velocity_app, velocity_ret):
    integrand_app = partial(Integrand, t=t_, phi=phi_, velocity=velocity_app)
    integrand_ret = partial(Integrand, t=t_, phi=phi_, velocity=velocity_ret)
    quad_app = quad(integrand_app, t1, t_max_)
    quad_ret = quad(integrand_ret, t_max_, t_)
    return quad_app[0] + quad_ret[0]


def Calculation_t1(time_ret, t_max__, phi__, velocity_app__, velocity_ret__):
    t1 = []
