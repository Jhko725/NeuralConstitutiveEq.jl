...
def modellist():  # list of all relaxation functions currently available
    modellist = [
        ...
        "dSLS",
        "springpot",
        "springpot-spring-parallel",
        "springpot-spring-parallel2",
        "sPLR",
        "springpot-dashpot-parallel",
        "sPLReta",
        "springpot-spring-serial",
        "springpot-spring-serial2",
        "springpot-dashpot-serial",
        "springpot-dashpot-serial2",
        "springpot-springpot-parallel",
        "springpot-springpot-serial",
        "springpot-springpot-serial2",
        "springpot-springpot-serial3",
        "fractionalSLS",
        "fractionalSLS2",
        "fractionalMWe",
        "fractionalMWe2",
...
def relaxation_function(par, model, Time):
    ...
    warnings.filterwarnings(
        "ignore", message="divide by zero encountered in reciprocal"
    )
    ...
    if model == "elastic" or model == "DMT":
        ...
        Einf = par[4]
        relfun = (
            lambda Es1, tau1, Es2, tau2, Einf, t: Es1 * exp(-t / tau1)
            + Es2 * exp(-t / tau2)
            + Einf
        )
        ...
        model == "springpot-spring-parallel2" or model == "sPLR"
    ):  # 1 -single spring-pot E1 global
        ...
        model == "springpot-dashpot-parallel"
        or model == "sPLReta"
        or model == "sPLRetatest"
    ):  # 1 'sPLReta'
        ...
        model == "springpot-spring-serial2"
    ):  # 'fractMW' fractional Maxwell model with tau
        ...
        Es0 = Ea1 * tau ** (-alpha)
        relfun = (
            lambda Es, alpha, tau, t: Ea1
            * tau ** (-alpha)
            * mlf(alpha, 1, -((t / tau) ** alpha))
        )
        ...
        relfun = (
            lambda Ea1, alpha, eta, t: eta ** (alpha / (alpha - 1))
            * Ea1 ** (1.0 / (1 - alpha))
            * t ** (-alpha)
            * mlf(1 - alpha, 1 - alpha, -(Ea1 / eta) * t ** (1 - alpha))
        )
        ...
        eta = Ea1 * tau ** (1 - alpha)
        relfun = (
            lambda Ea1, alpha, tau, t: Ea1
            * (t * tau) ** (-alpha)
            * mlf(1 - alpha, 1 - alpha, -((t / tau) ** (1 - alpha)))
        )
        Et = relfun(Ea1, alpha, tau, Time)
        ...
        Eb1 = par[2]
        betta = par[3]
        relfun = lambda Ea1, alpha, Eb1, betta, t: Ea1 * t ** (-alpha) + Eb1 * t ** (
            -betta
        )
        ...
        relfun = (
            lambda Ea1, alpha, Eb1, betta, t: Ea1 ** (betta / (betta - alpha))
            * Eb1 ** (alpha / (alpha - betta))
            * t ** (-betta)
            * mlf(alpha - betta, 1 - betta, -(Eb1 / Ea1) * t ** (alpha - betta))
        )
        Et = relfun(Ea1, alpha, Eb1, betta, Time)
        ...
        relfun = (
            lambda Ea1, alpha, betta, tau, t: Ea1
            * tau ** (-alpha)
            * t ** (-betta)
            * mlf(alpha - betta, 1 - betta, -((t / tau) ** (alpha - betta)))
        )
        ...
        relfun = (
            lambda Eb1, alpha, betta, tau, t: Eb1
            * (t * tau) ** (-betta)
            * mlf(alpha - betta, 1 - betta, -((t / tau) ** (alpha - betta)))
        )
        ...
        E0 = Es1 + Es2
        relfun = (
            lambda Ea1, alpha, Es1, Es2, t: Es1 * mlf(alpha, 1, -(Es1 / Ea1) * t**alpha)
            + Es2
        )
        ...
        relfun = (
            lambda E0, Einf, alpha, tau, t: (E0 - Einf)
            * mlf(alpha, 1, -((t / tau) ** alpha))
            + Einf
        )
        ...
        model == "fractionalMWe"
    ):  # fractional model, springpot-dashpot in parallel with spring
        ...
        tau = (eta / Ea1) ** (1 / (1 - alpha))
        relfun = (
            lambda Ea1, alpha, eta, Es1, t: eta ** (alpha / (alpha - 1))
            * Ea1 ** (1.0 / (1 - alpha))
            * t ** (-alpha)
            * mlf(1 - alpha, 1 - alpha, -(Ea1 / eta) * t ** (1 - alpha))
            + Es1
        ...
        eta = Ea1 * tau ** (1 - alpha)
        relfun = (
            lambda Ea1, alpha, tau, Es1, t: Ea1
            * (t * tau) ** (-alpha)
            * mlf(1 - alpha, 1 - alpha, -((t / tau) ** (1 - alpha)))
            + Es1
        )
        ...
        dTp = par[3]
        relfun = (
            lambda E0, Einf, dTp, alpha, t: Einf + (E0 - Einf) / (1 + t / dTp) ** alpha
        )
