...
def ting_numerical(
    ...
    if Height > 0:  # bottom effect correction
        [BEC, BECspeed] = bottom_effect_correction(
            Poisson, probe_size, Height, probe_geom, indentationfull
        )
    else:
    ...
    for i in range(1, PointN - 1):  # MaxInd or PointN-1
        ForceR[i] = K1 * (
            BEC[i] * np.trapz(Et[i::-1] * ind2speed[: i + 1], dx=dT)
            + power
            * eta
            * indentationfull[i] ** (power - 1)
            * indspeed[i]
            * BECspeed[i]
        )
    ...
    for i in range(MaxInd, endofalgorithm2 - 2):  # force for retraction part
        ...
        for j in range(b, localend - 1, -1):
            if localend == 0:
                ...
                res2[j] = (
                    np.trapz(Et[i - j :: -1] * indspeed[j : i + 1], dx=dT)
                    + eta * indspeed[i]
                )
                if res2[j] > 0:
                    localend = j
        ...
        ForceT[i] = K1 * (
            BEC[ijk] * np.trapz(Et[i : i - ijk - 1 : -1] * ind2speed[: ijk + 1], dx=dT)
        )
    if modelting == "DMT":
        ForceT += adhesion
...
if __name__ == "__main__":
    ...
    t1 = time.time()
    ForceT, cntrad, contact_time, t1_ndx, Et, ForceR = ting_numerical(
        par,
        Poisson,
        probe_size,
        dT,
