...
plot_kwargs = {"markersize": 2.5, "alpha": 0.8}
axes[0].plot(t_array, indentation_noise * 1e6, ".", color="navy", **plot_kwargs)
axes[1].plot(t_array, F_total_noise * 1e9, ".", color="darkred", **plot_kwargs)
axes[-1].set_xlabel("Time: $t$")
for ax, yname in zip(axes, ("Indentation: $I(t)$", "Force: $F(t)$")):
...
# %%
MSE_apptime_appparams = np.mean(np.square(F_app_curvefit - F_app)) / np.mean(
    np.square(F_app_curvefit)
)
MSE_apptime_retparams = np.mean(np.square(F_at_rp - F_app)) / np.mean(
    np.square(F_at_rp)
)
MSE_apptime_totparams = np.mean(np.square(F_at_tp - F_app)) / np.mean(
    np.square(F_at_tp)
)

MSE_rettime_appparams = np.mean(np.square(F_rt_ap - F_ret)) / np.mean(
    np.square(F_rt_ap)
)
MSE_rettime_retparams = np.mean(np.square(F_ret_curvefit - F_ret)) / np.mean(
    np.square(F_ret_curvefit)
)
MSE_rettime_totparams = np.mean(np.square(F_rt_tp - F_ret)) / np.mean(
    np.square(F_rt_tp)
)

MSE_tottime_totparams = np.mean(np.square(F_total_curvefit - force)) / np.mean(
    np.square(F_total_curvefit)
)
MSE_tottime_appparams = np.mean(np.square(force_app_parameter - force)) / np.mean(
    np.square(force_app_parameter)
)
MSE_tottime_retparams = np.mean(np.square(force_ret_parameter - force)) / np.mean(
    np.square(force_ret_parameter)
)
# %%
fig, ax = plt.subplots(1, 1, figsize=(7, 5))
...
each_MSE = {
    "approach time": np.array(
        [MSE_apptime_appparams, MSE_apptime_retparams, MSE_apptime_totparams]
    ),
    "retraction time": np.array(
        [MSE_rettime_appparams, MSE_rettime_retparams, MSE_rettime_totparams]
    ),
}
width = 0.5
