...
def SLS_constit_integand(t_, t, E0, E_inf, tau, velocity, indentation, tip):
    a = tip.alpha
    b = tip.beta
    return a * b * (E_inf + (E0 - E_inf)*np.exp(-(t-t_)/tau))* velocity(t_) * indentation(t_)**(b-1)
...
tau = 2
#%%
Force = F_app_integral(t__= time_app, E0_= E0, E_inf_=E_inf, tau_=tau, indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)

# Curve Fitting(PLR model) Approach Region
F_app_func = partial(F_app_integral, indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)
...
def Quadrature(t1, t_, t_max_, E0_, E_inf_, tau_, velocity_app, velocity_ret):
    integrand_app = partial(Integrand, E0=E0_, t=t_, E_inf=E_inf_, tau=tau_, velocity=velocity_app)
    integrand_ret = partial(Integrand, E0=E0_, t=t_, E_inf=E_inf_, tau=tau_, velocity=velocity_ret)
    quad_app = quad(integrand_app, t1, t_max_)
    quad_ret = quad(integrand_ret, t_max_, t_)
    return quad_app[0] + quad_ret[0]
#%%
def Calculation_t1(time_ret, t_max__, E0__, E_inf__, tau__, velocity_app__, velocity_ret__):
    t1 = []
    for i in time_ret:
    ...
    return t1
# %%
t1 = Calculation_t1(time_ret, t_max, E0, E_inf, tau, velocity_app_func, velocity_ret_func)
print(t1)
# %%
...
Force = F_ret_integral(t__= t1, t___ = time_ret, E0_= E0, E_inf_=E_inf, tau_=tau, indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)
...
def F_ret_integral_test(t__, E0_, E_inf_, tau_, indentation_, tip_, velocity_app, velocity_ret):
...
F_ret_integral_test(
    ...
    tip_=tip) 
# %%
F_ret_func = partial(F_ret_integral_test, indentation_=indentation_app_func, velocity_app=velocity_app_func, velocity_ret=velocity_ret_func, tip_=tip)
...
ax.set_ylabel("Force(nN)")
# %%
def F_total_integral(time, E0_, E_inf_, tau_, max_ind, indentation_, tip_, velocity_app, velocity_ret):
    time_app = time[:max_ind+1]
    time_ret = time[max_ind:]
    t1 = Calculation_t1(time_ret, time_ret[0], E0_, E_inf_, tau_, velocity_app, velocity_ret)
    F_app = []
    for i in time_app:
        integrand_ = partial(
            SLS_constit_integand, 
...
time = time[:length]
force = force[:length]
force_app_parameter = F_total_integral(time=time, max_ind=max_ind, E0_= popt_app[0], E_inf_=popt_app[1], tau_=popt_app[2], indentation_=indentation_app_func, velocity_app=velocity_app_func, velocity_ret=velocity_ret_func, tip_=tip)
force_ret_parameter = F_total_integral(time=time, max_ind=max_ind, E0_= popt_ret[0], E_inf_=popt_ret[1], tau_=popt_ret[2], indentation_=indentation_app_func, velocity_app=velocity_app_func, velocity_ret=velocity_ret_func, tip_=tip)
...
axes[2].plot(x, tau)
#%%
F_at_rp = F_app_integral(t__= time_app, E0_= popt_ret[0], E_inf_=popt_ret[1], tau_=popt_ret[2], indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)
F_at_tp = F_app_integral(t__= time_app, E0_= popt_total[0], E_inf_=popt_total[1], tau_=popt_total[2], indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)

t1_rt_ap = Calculation_t1(time_ret, t_max, popt_app[0], popt_app[1], popt_app[2], velocity_app_func, velocity_ret_func)
t1_rt_tp = Calculation_t1(time_ret, t_max, popt_total[0], popt_total[1], popt_total[2], velocity_app_func, velocity_ret_func)
F_rt_ap = F_ret_integral(t__ = t1_rt_ap, t___= time_ret, E0_= popt_app[0], E_inf_=popt_app[1], tau_=popt_app[2], indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)
F_rt_tp = F_ret_integral(t__ = t1_rt_tp, t___= time_ret, E0_= popt_total[0], E_inf_=popt_total[1], tau_=popt_total[2], indentation_=indentation_app_func, velocity_=velocity_app_func, tip_=tip)
# %%
