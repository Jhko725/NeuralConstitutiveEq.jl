...
def getH(lam, Gexp, wexp, H, kernMat, *argv):
    ...
    if len(argv) > 0:
        ...
        res_lsq = least_squares(
            residualLM, Hplus, jac=jacobianLM, args=(lam, Gexp, wexp, kernMat)
        )
    ...
    # send normal H, and collect optimized H back
        ...
        res_lsq = least_squares(
            residualLM, H, jac=jacobianLM, args=(lam, Gexp, wexp, kernMat)
        )
        return res_lsq.x
...
def getContSpec(par):
    ...
    if par["verbose"]:
        ...
        if par["plateau"]:
            ...
            np.savetxt(
                "output/H.dat", np.c_[s, H], fmt="%e", header="G0 = {0:0.3e}".format(G0)
            )
        else:
