...
##
function run_experiment(E, η, noise_amp, t_s, app_only=true)
    tip = Conical(π / 10.0)
    loading = Triangular(10.0, 0.2) # 10um/s, 0.2
    kv = KelvinVoigt(E, η)
    F(t) = force_kv(t, plr, loading, tip)
...
E_true, η_true = 1.614, 0.025
    f_s = 2e-3
    noise_amps = 0:0.002:0.04
    result_app = map(x -> run_experiment(E_true, η_true, x, f_s, true), noise_amps) |> x -> cat(x..., dims=3)
    result_total = map(x -> run_experiment(E_true, η_true, x, f_s, false), noise_amps) |> x -> cat(x..., dims=3)
    plot_E = scatter(noise_amps, result_app[1, 1, :], yerror=result_app[1, 2, :], label="Approach only", ylabel="E")
    scatter!(plot_E, noise_amps, result_total[1, 1, :], yerror=result_total[1, 2, :], label="Approach & Retract")
...
let
    ...
    f_s = 2e-3
    η_trues = 0.0:0.05:0.995
    result_app = map(x -> run_experiment(E_true, x, noise_amp, f_s, true), η_trues) |> x -> cat(x..., dims=3)
    result_total = map(x -> run_experiment(E_true, x, noise_amp, f_s, false), η_trues) |> x -> cat(x..., dims=3)
